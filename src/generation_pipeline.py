...
class ChunkWiseGenerationPipeline:
    ...
    def run(
        ...
        for index, section in enumerate(plan.sections):
            ...
            generated_prefix = self._build_recent_text(section_outputs)
            ...
            logger.info(f"[Pipeline] Section {index + 1} prompt: {prompt_len} chars, target_length={section.target_length}")
            ...
            if not section_text:
                ...
                logger.info(f"[Pipeline] Section {index + 1} generated: {actual_len} chars")

            length_warning = self._check_length(section=section, section_text=section_text)
            if length_warning:
                quality_report.section_warnings.append(length_warning)
                logger.warning(f"[Pipeline] Section {index + 1} length warning: {length_warning}")
    ...
    def _resolve_plan(
        ...
        logger.debug(f"Raw plan output from model:\n{raw_plan}")
        try:
            return GenerationPlan.from_json(raw_plan)
        except ValueError as exc:
            logger.error(f"Failed to parse plan: {exc}")
